...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        pos_x = int(round(pos.x()))
        pos_y = int(round(pos.y()))
        ...
        if self.drawing():
            ...
            if self.current:
                ...
                self.parent().window().label_coordinates.setText(
                    ...
                    % (
                        int(round(current_width)),
                        int(round(current_height)),
                        pos_x,
                        pos_y,
                    )
                )
    ...
    def wheelEvent(self, ev):
        ...
        if qt_version == 4:
            ...
            h_delta = delta.x()
            v_delta = delta.y()
            # Trackpads may report mostly pixelDelta, not angleDelta.
            if h_delta == 0 and v_delta == 0 and hasattr(ev, "pixelDelta"):
                pixel_delta = ev.pixelDelta()
                h_delta = pixel_delta.x()
                v_delta = pixel_delta.y()

        mods = ev.modifiers()
        ctrl_shift = Qt.ControlModifier | Qt.ShiftModifier
        if mods == ctrl_shift and v_delta:
            self.lightRequest.emit(v_delta)
        elif mods == Qt.ControlModifier and v_delta:
