...
class LabelFile(object):
    ...
    def save_create_ml_format(
        ...
        if image.isNull():
            raise LabelFileError(
                "Unable to load image while saving CreateML annotation."
            )
    ...
    def save_pascal_voc_format(
        ...
        if image.isNull():
            raise LabelFileError(
                "Unable to load image while saving PascalVOC annotation."
            )
        image_shape = [image.height(), image.width(), 1 if image.isGrayscale() else 3]
    ...
    def save_yolo_format(
        ...
        if isinstance(image_data, QImage):
            ...
            image.load(image_path)
        if image.isNull():
            raise LabelFileError("Unable to load image while saving YOLO annotation.")
        image_shape = [image.height(), image.width(), 1 if image.isGrayscale() else 3]
        writer = YOLOWriter(
            img_folder_name, img_file_name, image_shape, local_img_path=image_path
        )
    ...
    def convert_points_to_bnd_box(points):
        ...
        if x_min == float("inf") or y_min == float("inf"):
            raise LabelFileError(
                "Invalid shape points: no valid coordinates were found."
            )
